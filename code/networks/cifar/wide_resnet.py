...
class WideResNet(CifarModel):
    ...
    def network(self, img_input):
        ...
        def residual_block(img_input, out_filters, increase_filter=False):
            ...
            x = layers.Conv2D(out_filters, kernel_size=(3,3), strides=first_stride, padding='same', kernel_initializer=initializers.he_normal(), kernel_regularizer=regularizers.l2(self.weight_decay))(x)
            x = layers.BatchNormalization()(x)
            x = layers.Activation('relu')(x)

            x = layers.Conv2D(out_filters, kernel_size=(3,3), strides=(1,1), padding='same', kernel_initializer=initializers.he_normal(), kernel_regularizer=regularizers.l2(self.weight_decay))(x)
            
            if increase_filter or out_filters != 16:

                projection = layers.Conv2D(out_filters, kernel_size=(1,1), strides=first_stride, padding='same', kernel_initializer=initializers.he_normal(), kernel_regularizer=regularizers.l2(self.weight_decay))(img_input)
        ...
        def wide_residual_layer(x,out_filters,increase_filter=False):
            ...
            for _ in range(1, (self.depth - 4) // 6): x = residual_block(x, out_filters)

            return x

        x = conv3x3(img_input,     16)
        x = wide_residual_layer(x, 16*self.wide)
        x = wide_residual_layer(x, 32*self.wide, increase_filter=True)
        x = wide_residual_layer(x, 64*self.wide, increase_filter=True)
